import 'package:byte_client/app/pages/login/subpages/forget_password/forget_password_bloc.dart';
import 'package:byte_client/app/pages/login/subpages/signup/signup_bloc.dart';
import 'package:byte_client/app/pages/login/subpages/signin/signin_bloc.dart';
import 'package:byte_client/app/pages/login/login_bloc.dart';
import 'package:bloc_pattern/bloc_pattern.dart';
import 'package:flutter/material.dart';
import 'package:byte_client/app/pages/login/login_page.dart';

class LoginModule extends ModuleWidget {
  @override
  List<Bloc> get blocs => [
        Bloc((i) => ForgetPasswordBloc()),
        Bloc((i) => SignupBloc()),
        Bloc((i) => SigninBloc()),
        Bloc((i) => LoginBloc()),
      ];

  @override
  List<Dependency> get dependencies => [];

  @override
  Widget get view => LoginPage();

  static Inject get to => Inject<LoginModule>.of();
}
